...
**Коммерческая фича (на будущее): обработка ошибок/лимитов без paywall.**
- В EDI-Doc при окончании оплаты/лимитов их сервиса появляется попап с сырым JSON, например: `{"result":1,"message":"Request limit exceeded on Free plan. Please try again later."}`.
- В нашем продукте такого «Free plan/оплата» быть не должно (свой бэкенд/интеграция), но **нужен единый механизм ошибок** для UI:
  - Показывать человеку понятные сообщения для типовых случаев: 401/403 (неверные креды), 429 (лимит/слишком много запросов — «попробуйте позже»), 5xx/timeout (сервис недоступен), плюс общий случай.
  - Если ответ от нашего сервиса JSON и содержит `message` — показывать его (возможно с “Тех.деталями”), но без формулировок про «free plan».
  - Это важно, т.к. плагин планируется **как продукт для продажи**.

---

## 27.02.2026 — RoomBroomChainPlugin: «Настройки» как у EDI‑Doc + сохранение конфигов

**UI («Настройки»):**
- Вкладка «Настройки» приблизена к EDI‑Doc (DevExpress):
  - вместо выбора оператора: текст **«Ваш оператор ЭДО — Диадок»**
  - поля: логин/пароль/api token
  - тумблер: **«Создавать накладные с проведением»**
  - блок «Отчёты» (вкл/выкл) + текст-пояснение
  - убрано отображение **Root** (корневые layout-группы)

**Хранение настроек:**
- Настройки сохраняются и читаются локально из `SettingsMain` (JSON) через `ApplicationSettingsBase`.
- Код: `edo_iiko_bridge/RoomBroomChainPlugin/Config/ConfigStore.cs`, `Config/RoomBroomSettings.cs`, `Config/RoomBroomConfig.cs`.

**Упаковка:**
- `scripts/package.ps1` обновлён: можно собирать с `-IikoChainLibDir "C:\Program Files\iiko\iikoChain\Office"` или упаковать уже собранный DLL (если `Directory.Build.props` отсутствует, но DLL есть в `bin/Release`).

**Следующий шаг:**
- Вкладка «Документы»: кнопка «Получить входящие» использует сохранённые настройки; результаты (список/открытие) + единый показ ошибок (401/403/429/5xx).
