...
├── edo_iiko_bridge/
│   └── RoomBroomChainPlugin/    # Плагин iikoChain Office (ЭДО-мост Diadoc)
│       ├── Diadoc/              # HTTP-клиент Diadoc API
│       ├── Pages/               # UI: DocsPage, SettingsPage
│       ├── Config/              # Хранение настроек
│       └── scripts/package.ps1  # Сборка + ZIP
├── reports/
│   └── context-for-ai.md       # AI-контекст при смене устройства
├── etl.py                       # Основной ETL скрипт (загрузка из iiko в Neon)
├── alerts_bot.py                # Telegram-бот с алармами
...
- `transactions` / `transactions_products` — нормализованная лента, SIGNED-логика. Движение считается **без** списаний с контрагентом «Порча» (фильтр в transactions).
- `weekly_movement_products` — движение по товарам (базовая витрина).
- `weekly_product_documents_products` — документы (приходы, списания и т.д.) для дашборда; источник base — olap_postings, поэтому в таблице «Списания» отображаются в т.ч. списания по порче. Миграция: `docs/migrations/weekly_product_documents_include_spoilage.sql`.
...
1. **Результаты** — ТОП по отрицательным отклонениям (%); ТОП по положительным отклонениям (%); ТОП по недостачам ($); ТОП по излишкам ($); KPI; график динамики. Фильтр по диапазону, филиалу, товару, ТОП N.
2. **Анализ** — несохранённые, неверно посчитанные, пересорт, приёмка; приходы, списания (в т.ч. Порча), накладные, акты.
...
**ТОП N на вкладке «Результаты»:** для каждой из четырёх таблиц (отриц./положит. в % и в $) используется поле ранга и фильтр `[поле_ранга] <= INT([top_n_param])`. Формулы рангов для всех четырёх (в т.ч. `rank_by_surplus_money`, `rank_by_surplus_pct` для положительных) — в `.cursor/rules/datalens-iiko-knowledge.md`, раздел «ТОП N по отклонениям (фильтр по рангу)».
...
**Telegram‑бот с алармами (`alerts_bot.py`):**
- `TELEGRAM_BOT_TOKEN` — токен бота
- `TELEGRAM_CHAT_ID` — id чата, куда бот имеет право писать (для ограничения доступа)
- `ALERTS_TOP_N` — (опционально) сколько позиций показывать в ТОПах (по умолчанию 10)

## Локальный запуск

1. Установить зависимости: `pip install -r requirements.txt`
2. Создать `.env` файл с переменными окружения (см. выше)
3. Запустить ETL: `python etl.py`
4. Запустить бота с алармами: `python alerts_bot.py` (дальше в Telegram использовать команду `/week` для сводки по последней неделе)
...
**Реализовано:** ETL с ручным периодом и INVOICE; RAW → CORE → MART → DataLens; флаги неверного пересчёта и несохранённых позиций; три вкладки дашборда (Итоговая инвентаризация, Анализ пути товара, Песочница). Система даёт: системные отклонения, неверно пересчитанные и несохранённые позиции, путь документа, приходы/списания, анализ нескольких недель. Плагин RoomBroom для iikoChain Office: авторизация Diadoc, загрузка юрлиц/контрагентов/документов, сопоставление входящих документов с контрагентами, цветовая раскраска статусов.
